...
export default function CreatePoolPage() {
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.inititalBuyPrice || parseFloat(formData.inititalBuyPrice) <= 0) {
      toast.error("Please enter a valid initial buy price.");
      return false;
    }

    if (!formData.initialSellPrice || parseFloat(formData.initialSellPrice) <= 0) {
      toast.error("Please enter a valid initial sell price.");
      return false;
    }

    if (parseFloat(formData.inititalBuyPrice) <= parseFloat(formData.initialSellPrice)) {
  ...
  const handleCreatePool = async () => {
    ...
    try {
      ...
      if (result.success) {
        toast.success(`Pool created successfully! Transaction hash: ${result.txHash}`);
        // Reset form
        setFormData({
          token: "",
          ethAmount: "",
  ...
  const estimatedSpread = formData.inititalBuyPrice && formData.initialSellPrice 
    ? ((parseFloat(formData.inititalBuyPrice) - parseFloat(formData.initialSellPrice)) / parseFloat(formData.inititalBuyPrice) * 100).toFixed(2)
    : "0";
        ...
        <div className="max-w-2xl mx-auto space-y-8">
          ...
          <div className="relative rounded-xl p-8 overflow-hidden backdrop-blur-xl border border-white/[0.05]">
            ...
            <div className="relative">
              ...
              <p className="text-muted-foreground/70 font-plus-jakarta">
                Initialize a new liquidity pool with your token. Set initial prices and provide liquidity to start trading.
              </p>
            </div>
          </div>
          ...
          <div className="relative rounded-xl overflow-hidden backdrop-blur-xl border border-white/[0.05]">
            ...
            <Card className="relative border-0 bg-transparent shadow-none">
              <CardHeader>
                <CardTitle className="text-xl font-semibold text-white">Pool Configuration</CardTitle>
              ...
              <CardContent className="space-y-6">
                ...
                <div className="space-y-2">
                  <Label htmlFor="token" className="text-white">Token Contract Address</Label>
                  <div className="relative">
                    <Input
                      id="token"
                      placeholder="0x..."
                  ...
                  {tokenInfo && (
                    <div className="p-3 rounded-lg bg-primary/10 border border-primary/20">
                      <p className="text-sm text-white">
                        <span className="font-medium">{tokenInfo.symbol}</span> - {tokenInfo.name}
                      </p>
                      <p className="text-xs text-muted-foreground">Decimals: {tokenInfo.decimals}</p>
                    </div>
                  )}
                </div>
                ...
                <div className="space-y-4">
                  <h3 className="text-lg font-medium text-white">Initial Price Configuration</h3>
                  
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      <Label htmlFor="buyPrice" className="text-white">Initial Buy Price (ETH per Token)</Label>
                      <Input
                        id="buyPrice"
                        type="number"
                        step="0.000001"
                        ...
                        onChange={(e) => handleInputChange("inititalBuyPrice", e.target.value)}
                      />
                    </div>

                    <div className="space-y-2">
                      <Label htmlFor="sellPrice" className="text-white">Initial Sell Price (ETH per Token)</Label>
                      <Input
                        id="sellPrice"
                        type="number"
                        step="0.000001"
                        ...
                        onChange={(e) => handleInputChange("initialSellPrice", e.target.value)}
                      />
                    </div>
                  </div>
                  ...
                  {formData.inititalBuyPrice && formData.initialSellPrice && (
                    <div className="p-4 rounded-lg bg-secondary/10 border border-secondary/20">
                      <h4 className="text-sm font-medium text-white mb-2">Price Summary</h4>
                      <div className="grid grid-cols-2 gap-4 text-sm">
                        <div>
                          <p className="text-muted-foreground">Buy Price:</p>
                          <p className="text-green-400 font-medium">{formData.inititalBuyPrice} ETH</p>
                        </div>
                        <div>
                          <p className="text-muted-foreground">Sell Price:</p>
                          <p className="text-red-400 font-medium">{formData.initialSellPrice} ETH</p>
                        </div>
                        <div className="col-span-2">
                          <p className="text-muted-foreground">Spread:</p>
                          <p className="text-white font-medium">{estimatedSpread}%</p>
                        </div>
                      </div>
                    </div>
                  )}
